...
def test_check_subscribe_checkbox(browser, register_account_page_url):
    ...
    subscribe_checkbox = wait.until(
        method=EC.visibility_of_element_located((By.ID, "input-newsletter")),
...
# 4 Ввод почты
def test_check_email(browser, register_account_page_url):
    browser.get(register_account_page_url)
    wait = WebDriverWait(browser, 2, poll_frequency=1)
    email = wait.until(
        method=EC.visibility_of_element_located((By.NAME, "email")),
...
def test_check_header(browser, register_account_page_url):
    ...
    wait = WebDriverWait(browser, 2, poll_frequency=1)
    header = wait.until(
        method=EC.visibility_of_element_located((By.XPATH, '//*[@id="content"]/h1')),
